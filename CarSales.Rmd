...
hist(data, main="Raw Data Histogram", xlab="Car Sales")
...
ts.plot(log(train_data), main = "Log Transformed Data", ylab = expression(Y[t]))
```
...
fit2_m1 <- arima(log(train_data), order=c(2,1,2), 
                seasonal=list(order=c(1, 1, 0), period=12), 
                fixed=c(0, 0, NA, 0, NA), method="ML"); fit2_m1 #just show this
```
\
...
for (p in 0:4) {
  for (q in 0:4) {
    aiccs[p + 1, q + 1] = AICc(arima(log(train_data), order = c(p, 1, q), method = "ML"))
  }
}
aiccs
...
```{r, echo=FALSE, out.width="50%", warning=FALSE, message=FALSE, error=FALSE, results='hide'}
ma_coef <- c(1, -0.8602)
seasonal_ar_coef <- c(1, -0.3685)
uc.check(pol = ma_coef, plot_output = TRUE) #roots of MA part of model A
uc.check(pol = seasonal_ar_coef, plot_output = TRUE) #roots of SAR part of model A
```
...
plot.ts(res,ylab= "Residuals",main="Residuals Plot")
...
ts.plot(log(train_data), xlim=c(1,length(log(train_data))+10), 
        ylim = c(min(log(train_data)),max(U.tr)))
lines(U.tr, col="blue", lty="dashed")
lines(L.tr, col="blue", lty="dashed")
points((length(log(train_data))+1):(length(log(train_data))+10), pred.tr$pred, col="red")
```
...
ts.plot(as.numeric(data), xlim = c(98,length(train_data)+10), ylim = c(200,max(U)),col="red",ylab="Car Sales",main="Zoomed in visualization of forecasting on
testing set")
...
```{r, echo=TRUE, fig.show='hide', results='hide', warning=FALSE, collapse=TRUE}
library(forecast)
library(qpcR)
library(tidyr)
library(tsdl)
